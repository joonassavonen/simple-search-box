...
(function () {
  ...
  function buildWidget() {
    ...
    function renderContactHtml(cfg, lang) {
      ...
      if (cfg.phone) {
        html += `<a href="tel:${escHtml(cfg.phone)}" class="findai-contact-btn findai-contact-phone">📞 Soita ${escHtml(cfg.phone)}</a>`;
      }
      if (cfg.chat_url) {
        html += `<a href="${escHtml(cfg.chat_url)}" target="_blank" rel="noopener" class="findai-contact-btn findai-contact-chat">💬 Lähetä WhatsApp-viesti</a>`;
      }
      if (cfg.email) {
        html += `<a href="mailto:${escHtml(cfg.email)}" class="findai-contact-btn findai-contact-email">✉️ ${escHtml(cfg.email)}</a>`;
      }
      html += "</div>";
      return html;
    }
